	...
	return Controller.extend('geosort.controller.projects.ProjectCreate', {
		...
		onSaveProject: async function (evt) {
			if (!this.validateSettings()) {
				const item = this.getView().byId('formContainer').getItems()[0]

				if (item instanceof MessageStrip) return
				const messageStrip = new MessageStrip({
					type: 'Error',
					showIcon: true,
					text: this.i18n.getText('MSG_STRIP_VALIDATION_ERROR')
				})
				messageStrip.addStyleClass('messageStrip')
				this.getView().byId('formContainer').insertItem(messageStrip, 0)
				return
			}
